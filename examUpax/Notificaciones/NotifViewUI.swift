...
class NotifViewUI: UIView{
    weak var delegate: NotifViewDelegate?
    var user: String?
   
    var infoData: NotificationEntity?{
        didSet{
            tabla.reloadData()
        }
    }
    ...
    lazy var navigationBar: ComponentNavbarHome = {
        ...
        navigationController.titleSection.text = "Notifications"
    ...
    lazy var tabla: UITableView = {
        ...
        tableview.register(notificationsCell.self, forCellReuseIdentifier: "notifCell")
...
extension NotifViewUI: UITableViewDelegate, UITableViewDataSource{
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        
        return infoData?.notif.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        
        let cell = tableView.dequeueReusableCell(withIdentifier: "notifCell", for: indexPath) as! notificationsCell
       
        let imageProfile = URL(string: (infoData?.notif[indexPath.row].imgperfil)!)!
        
        if let imagenPerfil = try? Data(contentsOf: imageProfile) {
            cell.imageProfile.image = UIImage(data: imagenPerfil)
        }
       
        cell.titleDescription.text = infoData?.notif[indexPath.row].titulo
        cell.titlemain.text = infoData?.notif[indexPath.row].descripcion
        return  cell
    }
